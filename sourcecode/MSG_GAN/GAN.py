...
class Generator(th.nn.Module):
    ...
    def forward(self, x):
        ...
        return self.choose_middle_output(outputs)

    def choose_middle_output(self, outputs):
        index = self.index
        assert 0 < index < self.depth-1, "wrong index"
        for i in range(self.depth):
            if i < index:
                outputs[index] += F.interpolate(outputs[i], size=(2**(index+2), 2**(index+2)))
            if index < i < self.depth-1:
                outputs[index] += F.avg_pool2d(outputs[i], 2 ** (i-index), stride=2)
        # print(outputs[index].shape) #32
        return outputs[index] / (self.depth-1), outputs[-1]
...
class Discriminator(th.nn.Module):
    ...
    def __init__(self, depth=7, feature_size=512,
        ...
        if self.index < 3:
            self.middle_converter = from_rgb(self.feature_size // 2)
        else:
            self.middle_converter = from_rgb(self.feature_size // np.power(2, self.index - 1))
        ...
        for i in range(self.depth - 1):
            if i > 2:
                ...
                layer = DisGeneralConvBlock(self.feature_size, self.feature_size,
                                            use_eql=self.use_eql)

            self.layers.append(layer)
            #self.rgb_to_features.append(rgb)

        # change the layers to adapt the middle connection
        if self.index < 5:
            self.layers[self.index-1] = DisGeneralConvBlock(self.feature_size // 2, self.feature_size,
                                            use_eql=self.use_eql)
            self.layers[self.index] = DisGeneralConvBlock(
                    int(self.feature_size // np.power(2, self.index - 2)),
                    int(self.feature_size // np.power(2, self.index - 2)),
                    use_eql=self.use_eql
                )
        else:
            # print(self.layers[self.index])
            # pdb.set_trace()
            self.layers[self.index] = DisGeneralConvBlock(
                    int(self.feature_size // np.power(2, self.index - 2)),
                    int(self.feature_size // np.power(2, self.index - 1)),
                    use_eql=self.use_eql
                )

        # just replace the last converter ?
        self.first_rgb_to_features = \
    ...
    def forward(self, inputs):
        ...
        y = self.first_rgb_to_features(inputs[-1]) # get features
        y = self.layers[self.depth - 2](y)

        for block in reversed(self.layers[:-1]):
            #input_part = converter(x)  # convert the input:
            #y = th.cat((input_part, y), dim=1)  # concatenate the inputs:
            if y.shape == inputs[0].shape:
                input_part = self.middle_converter(inputs[0])
    ...
    def train(self, data, gen_optim, dis_optim, loss_fn, normalize_latents=True,
        ...
        for epoch in range(start, num_epochs + 1):
            ...
            for (i, batch) in enumerate(data, 1):
                ...
                images = [images] + [avg_pool2d(images, 4)]
                images = tuple(reversed(images)) # shape ?
